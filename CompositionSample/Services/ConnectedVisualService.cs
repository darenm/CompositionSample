...
namespace CompositionSample.Services
{
    public class ConnectedVisualService
    {
        ...
        private void VisualImageOnImageLoaded(object sender, EventArgs eventArgs)
        {
            ...
            GeneralTransform coordinate = visualImage.TransformToVisual(_hostFrame);
            Point position = coordinate.TransformPoint(new Point(0, 0));

            // setup initial visual position
            Vector3 currentOffset = _floatingVisual.Offset;
            ...
            var destinationOffset = new Vector3((float)position.X + (float)visualImage.ImageMargin.Left, (float)position.Y + (float)visualImage.ImageMargin.Top, 0);
            var destinationHorizontalRatio = ((CompositionSurfaceBrush)visualImage.ForegroundVisual.Brush).HorizontalAlignmentRatio;
            ...
            ((CompositionSurfaceBrush)_floatingVisual.Brush).StartAnimation(nameof(CompositionSurfaceBrush.HorizontalAlignmentRatio), horizontalRatioAnimation);
        ...
        public void DetachVisual(VisualImage source)
        {
            ...
            GeneralTransform coordinate = source.TransformToVisual(_hostFrame);
            Point position = coordinate.TransformPoint(new Point(0, 0));

            // Set the sprite to that offset relative to the host
            _floatingVisual.Offset = new Vector3((float)position.X + source.ForegroundVisual.Offset.X, (float)position.Y + source.ForegroundVisual.Offset.Y, 0);
